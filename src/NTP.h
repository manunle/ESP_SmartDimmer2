...
#include "SunSet.h"
SunSet sun;
#define LATITUDE        38.64595
#define LONGITUDE       120.51827
...
void SetSuriseset()
{
  ...
  {
    suntime.valid = false;
    Serial.println("sunset: date time not set");
    return;
  }
  double sunrise = sun.calcSunrise();
  double sunset = sun.calcSunset();
  suntime.riseHour=(int)sunrise/60;
  suntime.riseMin=sunrise-(suntime.riseHour*60);
  suntime.setHour=(int)sunset/60;
  suntime.setMin=sunset-(suntime.setHour*60);
  if(suntime.riseHour > 3 && suntime.riseHour < 10)
    suntime.valid = true;
  Serial.println("sunrise = " + String(suntime.riseHour) + ":" + String(suntime.riseMin));
  Serial.println("sunset = " + String(suntime.setHour) + ":" + String(suntime.setMin));
  Serial.println(String(sunrise));
  Serial.println(String(sunset));
  Serial.println(String(DateTime.year)+"/"+String(DateTime.month)+"/"+String(DateTime.day));
}
