...
#define DEVICE_TYPE "Esp01Relay"

//char tmpESP[100];
void mqttCallback(char* topic, byte* payload, unsigned int length);
void mqttSubscribe();
String verstr = "ESP01_SingleRelay ver 1.0";
  ...
  if(WIFI_connected)
  {
    ...
    if(config.MQTTServer != "")
    {
      ...
      if(cHeartbeat >= config.HeartbeatEvery and config.HeartbeatEvery > 0)
      {
        ...
        message = message + " " + String(config.RSchedule[0].onHour) + ":" + String(config.RSchedule[0].onMin) + " " + String(config.RSchedule[0].offHour) + ":" + String(config.RSchedule[0].offMin); 
        message = message + " " + String(config.RSchedule[1].onHour) + ":" + String(config.RSchedule[1].onMin) + " " + String(config.RSchedule[1].offHour) + ":" + String(config.RSchedule[1].offMin); 
